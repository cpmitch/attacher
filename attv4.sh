...
# 9-Apr-2014:
# Major overhaul. Version 2.
# The processLine {} function has been modified greatly, and a new sandbox-non directory defined
# to allow this script to 'play nice' with the scanv script. Previosuly, non would corrupt the
# scanv ability to see when a known client would appear after a  long absence. For example when
# someone with a new device that belongs next door was added to the scanv _attached.txt file,
# yet still had not associated there would be detected by non, it would reset the freshness
# marker and mask scanv's ability to report the return event.
#
# 20-Apr-2014 (Happy Easter!)
# Version 5 attempts to save CPU and time by paring down some redundant, confusing code especially
# in the main part of the script. It's quite messy around there, and conflicting things are happening
# in the processLine sub-routine.
#
# 28-Sep-2014:
# Happy Birthday Abby & Aiden, be safe on those new electric scooters!
# Adding in the MAC address to all master_log.txt reports to prepare for better post-processing abilities.
#
# 5-Nov-2014:
# Cloned from nonv6.sh this program now becomes attv1.sh in order to explore the idea of tracking
# clients who are attached to APs but otherwise fly under the radar. attv1.sh is meant to capture
# the presence or absence of clients who associate regularly with any AP not directly being monitored
# by the scanv script.
#
# 7-Nov-2014:
...
#
# 1-Apr-2015:
# Attempting to make the behavior green-list based. That is, unless the AP MAC is in the greenlist.txt file, attv2 will not report in e-mail.
#
# 21-Apr-2015:
...
#
# 28-Aug-2015:
# Back code segment does not correctly detect and report clients when a "ripe" (long running) airodump-ng process is halted and restarted.
# The code checks LASTSEENDATE and ISFRESH, triggering Back if both those variables indicate. Now I'm experimenting leaving ISFRESH condition out of the decision process.
# Well, nice try Jack! That did not work! Every attached client is now being marked as Back. Guess you're gonna need some different logic.
#
# 1-Sep-2015:
# Added _history.txt file for each MAC addr seen, similar to what was done in non code.
#
# 6-Sep-2015:
# Added day indicator to master_log.txt output
#
# 10-Sep-2015:
# Added code to capture historical data in the $SANDBOX/MAC_history.txt file.
#
# 12-Sep-2015:
# Incremented to the next version based upon brand new weedOutLine sub-routine that help to pare down the huge list of client MACs that are searched. 
# Current hard-coded threshold of 180 minutes means and MAC's LAst Seen Time that exceeds threshold will not be subject to processLine.
# Alrady it's observed that this will cut down on CPU usage when the airodump-ng csvfile is relatively new, but should also help in times
# when I cannot be there to reset the file every 7 days.
#
# 19-Sep-2015:
# Added code to capture historical data in the $SANDBOXAP/APMAC_history.txt file. With this a compelte history of clients can be collected on a per-AP basis.
#
# 25-Sep-2015:
# Added Roamed-Here triggering code. Now we actually can see when clients roam here & there as when scan code was mainly used.
#
# 6-Oct-2015:
# Added robustness to the detection and reporting of combo AP/client targets. Previously, we would assign CSVSTRING in a manner that would neglect
# the correct entry in csvfile. Now we are much more particular about what we grab and analyze.
#
# 12,13-Oct-2015:
# Added some new code to report Newly-Arrived clients' hisotry from the non sandbox. So now if a device shows up attaching for the first time, the new
# code triggers a check of the existence of the sandbox-non history file, and if found returns the first 4 fields of the top most line from it.
#
# 15-Oct-2015:
# Slight change in the way a text is generated after Newly-Arrived with Non history. It has been shortened to better display on google voice.
# Also slight change to the line PREVASSOCMACNAME= using egrep instead of the tired old cat, grep, awk string of commands. A few false positives
# are dragging me down here.
#
# 24-May-2016:
# Happy BDay Susan!
# Added (gasp!) logging to log.txt file.
#
# 21-Sep-2016:
# Incremented version to 4 since I'm trying to solve the Roamer to and from reporting issues. This could go terribly, horribly wrong...
#
#  1-Oct-2016 Bye bye Sprint, we're on Cricket wireless. Changes needed for TXT alerts.
#
# 29-Nov-2016:
# Adding code to launch new testv14.sh line script against any new clients that latch to greenlisted APs.
# New code will examine file ap_greenlist.txt and also check is a line script already running before spawning a new porces in new xterm.
#
# 21-Dec-2016:
# Added a whole bunch of these if hooks:
#              if [ -e $SANDBOXNON/$F1"_history.txt" ] ; then
# In order to document in the sandbox-non directory everything the newly-arrived client does. This will help document the entire history
# of the client where it started: in the sandbox-non directoy as well as having it the sandbox dir.
#
# 30-Dec-2016:
# Added FIRSTPASS variable and delay in weedOutLine subroutine to slow things down after ... the first pass.
#
#  7-Jan-2016:
# Out of fear or anger, I added a new CKSUM loop to detect the moment the csvfile is written. So it looks for CKSUM1 -ne CKSUM2, sleeps
# 1 second, and then goes to build the list of MACs that are attached to an AP. Stupid, right? It's just that attv4 DID NOT DETECT luna's
# re-arrival on WIRE777 when I went to the attic and turned it on after some 250 days today. Arrrrgggggghhhhhh.
#
# 27-May-2017:
# Found the bug that printed a line like this:
# +ROAM Sun 01/15/17 12:01:22 + attv4.sh (ATT9hMM9WB) 1C:56:FE:6F:39:42_unkMAC 1C:56:FE:6F:39:42 OUI Moto Lenovo Roamed To ATT9hMM9WB from ././sandbox-ap/F8:18:97:7F:4C:A6_current_essid.txt, csvfile nyx
#                                                                                                                                          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# So now hopefully that's behing us.
#
# 15-Oct-2017
# Added CHECKFORPAUSE and CHECKFORSTOP sub-routines.
#
# 19-Dec-2017 (lucky me!)
# Added ISCLIENTOUIBLACKLISTED to filter out annoying Raj AP NETGEAR locally administered MACs that keep showing up multiple times a day.
#
#  2-Oct-2018:
# Changed around the ROAM notifications a bit ti visually display <<  << when roamer detected. Also commented out 'These two should be dissimilar' logging.
#
# 22-Jan-2019:
# Changed the CHECKAPGREENLIST sub-routine to check for commented AP's in file $HOMEP/ap_greenlist.txt and if so, do not match. This basically allows me
# to comment out AP from the file without deleting them entirely.
#
# 24-Mar-2019:
# Added variable 6480MINUTES to properly un-hardcode 4.5 days in the back scripting.
#
# 18-Nov-2019:
# Now spawning testv14 with nohup to prevent parent process death from killing child.
#
#  2-Jan-2020:
